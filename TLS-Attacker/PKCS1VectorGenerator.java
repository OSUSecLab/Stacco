...
public final class PKCS1VectorGenerator {
     ...
     * @return
     */
    public static byte[][] generatePkcs1Vectors(RSAPublicKey publicKey, BleichenbacherCommandConfig.Type type, int error) {
	...
	if (type == BleichenbacherCommandConfig.Type.CORRECT) {
		keyBytes[0] = 0x03;
		keyBytes[1] = 0x03;
		byte[][] plainPaddedKeys = new byte[1][];
		plainPaddedKeys[0] = getPaddedKey(rsaKeyLength, keyBytes);

		try {
			Cipher rsa = Cipher.getInstance("RSA/NONE/NoPadding");
		    rsa.init(Cipher.ENCRYPT_MODE, publicKey);
		    byte[][] encryptedKeys = new byte[1][];
		    encryptedKeys[0] = rsa.doFinal(plainPaddedKeys[0]);
	...
	if (type == BleichenbacherCommandConfig.Type.INCORRECT) {
		byte[][] plainPaddedKeys = new byte[1][];
		switch (error) {
			case 1:
				...
				break;
			case 2:
				plainPaddedKeys[0] = getEK_NullByteInPadding(rsaKeyLength, keyBytes);
				break;
			case 3:
				plainPaddedKeys[0] = getEK_NullByteInPkcsPadding(rsaKeyLength, keyBytes);
				break;
			case 4:
				plainPaddedKeys[0] = getEK_SymmetricKeyOfSize(rsaKeyLength, keyBytes, 0);
				break;
			case 5:
				plainPaddedKeys[0] = getEK_SymmetricKeyOfSize(rsaKeyLength, keyBytes, 1);
				break;
			case 6:
				plainPaddedKeys[0] = getEK_SymmetricKeyOfSize(rsaKeyLength, keyBytes, 8);
				break;
			case 7:
				plainPaddedKeys[0] = getEK_SymmetricKeyOfSize(rsaKeyLength, keyBytes, 16);
				break;
			case 8:
				plainPaddedKeys[0] = getEK_SymmetricKeyOfSize(rsaKeyLength, keyBytes, 32);
				break;
			case 9:
				plainPaddedKeys[0] = getEK_WrongFirstByte(rsaKeyLength, keyBytes);
				break;
			case 10:
				plainPaddedKeys[0] = getEK_WrongSecondByte(rsaKeyLength, keyBytes);
				break;
			default:
				plainPaddedKeys[0] = getPaddedKey(rsaKeyLength, keyBytes);
				break;
