...
public class TlsRecordBlockCipher extends TlsRecordCipher {
     ...
     * @return
     */
    public int calculatePaddingLength(int dataLength, int expectedBlockNumber) {
    	int blocks = dataLength / encryptCipher.getBlockSize() + 1;
    	if (expectedBlockNumber > blocks)
			return encryptCipher.getBlockSize() * (expectedBlockNumber + 1 - blocks) 
				- (dataLength % encryptCipher.getBlockSize());
		else
